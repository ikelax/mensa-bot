...
function test(mealplans) {
  ...
  return counters
    .map((counter) => {
      const counterTitle = `__*${counter.displayName} - ${counter.description}*__\n`;
...
async function getMealplans() {
  ...
  const escapedMealplans = test(mealplans).replaceAll('-', '\\-').replaceAll('(', '\\(').replaceAll(')', '\\)');
  const link = "[Speiseplan](https://mensaar.de/#/menu/sb)"
  return escapedMealplans + "\n" + link
}

const bot = new Bot("5191974791:AAGz7Tns69X3L4ZN1g8ptrLQaNuYWUFdu64");
